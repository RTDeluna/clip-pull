...
# jobs, and simultaneous in-flight requests to OpenAI per job -- both start
# conservative since a user's actual OpenAI rate-limit tier isn't knowable
# in advance.
...
class TranscriptionOrchestrator:
    def __init__(
        ...
        openai_client_cls=OpenAIWhisperClient,
        ...
        self.openai_client_cls = openai_client_cls
        ...
        try:
            async with self._job_semaphore:
                ...
                if not openai_key:
                    self._fail(
                        history_id,
                        "Add an OpenAI API key in Settings to enable transcription.",
                    )
                    return
                ...
                try:
                    chunk_results = await self._transcribe_chunks(
                        history_id, chunks, openai_key, loop
                    )
    ...
    async def _transcribe_chunks(
        self, history_id: int, chunks: list[Path], openai_key: str, loop: asyncio.AbstractEventLoop
    ) -> list[dict]:
        openai_client = self.openai_client_cls(openai_key)
                ...
                for attempt in range(CHUNK_RETRY_ATTEMPTS):
                    try:
                        return await loop.run_in_executor(
                            None, openai_client.transcribe_chunk, chunk_path
                        )
                    except AIClientError as exc:
                        last_exc = exc
